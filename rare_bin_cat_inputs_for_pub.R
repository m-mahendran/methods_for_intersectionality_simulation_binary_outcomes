################################################################################
############ Rare binary outcome, all categorical inputs simulation ############
################################################################################
...
#############functions for simulation
  ...
  #bimodal dist for coeff
    Sample.negative.coeff <- function (n) {
      y0 <- rtruncnorm(n,a=0.20, b=0.76, mean=((0.76+0.2)/2), sd = 0.30)
    }
    
    Sample.positive.coeff <- function (n) {
      y0 <- rtruncnorm(n,a=1.24, b=1.80, mean=((1.24+1.80)/2), sd = 0.30)
    }
...
for (n in n_list){
  ...
  MADtable = NULL
  MADtable <- matrix(nrow=1000, ncol=1778)
  
  results = NULL
  results <- matrix(nrow=1000, ncol=1778)
  
  results <- foreach (i=1:1000, .combine=rbind, .packages=c('rpart', 'partykit', 'tuneRanger', 'lme4', 'lsr', 'truncnorm', 'mlr', 'SurrogateMinimalDepth', 'merTools', 'CHAID', 'plyr', 'sandwich', 'lmtest')) %dopar% {
    ...
    int <- -3
    b1.1 <- log(Sample.positive.coeff(1))
    b1.2 <- log(Sample.positive.coeff(1))
    b1.3 <- log(Sample.positive.coeff(1))
    b2 <-  log(Sample.positive.coeff(1))
    b3 <- log(Sample.negative.coeff(1))
    b4 <- log(Sample.negative.coeff(1))
    b5 <- log(Sample.negative.coeff(1))
    b6 <- 0  
    bint12 <- log(Sample.negative.coeff(1))
    bint13 <- log(Sample.negative.coeff(1))
    ...
    #CHAID
    CHAID <- chaid(y ~ x1 + x2 +x3 +x4 +x5 + x6, data = D)
    splitvarCHAID <- paste(names(varimp(CHAID)), collapse=".") 
    x6CHAID <- ifelse(grepl("x6", splitvarCHAID)=="TRUE", 1, 0) 
    ...
    CHAID.predict <- predict(CHAID, Truth, type="prob")[,2]
    CHAID.predict.results <- create_binom_predictions(CHAID.predict, "CHAID")
  ...
  write.csv(results,paste("Bin_rare_cat", n, "csv", sep = "."))   
  
  #MADTABLE: 1778 in width
