...
################################################################################
  ...
  for (n in n_list){
    ...
    results <- foreach (i=1:1000, .combine=rbind, .packages=c('rpart', 'partykit', 'tuneRanger', 'lme4', 'lsr', 'truncnorm', 'mlr', 'merTools', 'CHAID', 'plyr', 'sandwich', 'lmtest')) %dopar% {
      ...
      MADtable = c(i,preval, int, b1.1, b1.2, b1.3, b2, b3, b4, b5, bint12, bint13, bint2,
                   BF.predict.results,  MF.predict.results,
                   MLM.predict.results, CC.predict.results)
      
    }
    
    write.csv(results,paste("Bin_50_preval", n, "csv", sep = ".")) 
...
################################################################################
########### Common binary outcome, all categorical inputs simulation############
################### large effects sizes only for interactions ##################
################################################################################

  #############functions for simulation
    
    #bimodal dist for coeff
    Sample.main.coeff <- function (n) {
      y0 <- rtruncnorm(n,a=0.20, b=0.89, mean=((0.89+0.2)/2), sd = 0.30)
    }
    
    Sample.interaction.coeff <- function (n) {
      y0 <- rtruncnorm(n,a=1.11, b=1.80, mean=((1.11+1.80)/2), sd = 0.30)
    }
  ...
  for (n in n_list){
    ...
    results <- foreach (i=1:1000, .combine=rbind, .packages=c('rpart', 'partykit', 'tuneRanger', 'lme4', 'lsr', 'truncnorm', 'mlr', 'merTools', 'CHAID', 'plyr', 'sandwich', 'lmtest')) %dopar% {
      ...
      # natural log the bimodal function, so can create RR once exponentiated
      int <- -1.25
      b1.1 <- (Sample.main.coeff(1))
      b1.2 <- (Sample.main.coeff(1))
      b1.3 <- (Sample.main.coeff(1))
      b2 <-  (Sample.main.coeff(1))
      b3 <- (Sample.main.coeff(1))
      b4 <- (Sample.main.coeff(1))
      b5 <- (Sample.main.coeff(1))
      b6 <- 0  
      bint12 <- (Sample.interaction.coeff(1))
      bint13 <- (Sample.interaction.coeff(1))
      bint2 <- (Sample.interaction.coeff(1))
    ...
    write.csv(results,paste("Bin_50_preval_v2", n, "csv", sep = ".")) 
...
createTruth <- function(Int, B1, B2, B3, B4, B5 ,Bint1, Bint2) {
  ...
  Truth$pr = 1/(1+exp(-Truth$z))  # pass through log function
...
for (n in n_list){
  ...
  results <- foreach (i=1:1000, .combine=rbind, .packages=c('rpart', 'partykit', 'tuneRanger', 'lme4', 'lsr', 'truncnorm', 'mlr', 'merTools', 'CHAID', 'plyr')) %dopar% {
    ...
    MADtable = c(i,preval, int,  b1, b2, b3, b4, b5, bint1, bint2,
                 BF.predict.results, MF.predict.results, 
  ...
  write.csv(results,paste("Bin_50_preval_mixed", n, "csv", sep = "."))   
...
for (n in n_list){
  ...
  results <- foreach (i=1:1000, .combine=rbind, .packages=c('rpart', 'partykit', 'tuneRanger', 'lme4', 'lsr', 'truncnorm', 'mlr', 'merTools', 'CHAID', 'plyr')) %dopar% {
    ...
    # natural log the bimodal function, so can create RR once exponentiated
    int <- -1
    b1 <- (Sample.main.coeff(1))
    b2 <- (Sample.main.coeff(1))
    b3 <- (Sample.main.coeff(1))
    b4 <- (Sample.main.coeff(1))
    b5 <- (Sample.main.coeff(1))
    b6 <- 0  
    bint1 <- (Sample.interaction.coeff(1))
    bint2 <- (Sample.interaction.coeff(1))
    
    D$z <- int + b1*D$x1 + b2*D$x2 + b3*D$x3 + b4*D$x4 + b5*D$x5 + + bint1*D$int1*D$x1*D$x2 + bint2*D$int2*D$x3*D$x4*D$x5
    D$pr =  1/(1+exp(-D$z))   # pass through log function
    D$y = rbinom(n,1,D$pr) 
    preval <- mean(D$y)
  ...
  write.csv(results,paste("Bin_50_preval_mixed_v2", n, "csv", sep = "."))   
